#include <pebble.h>
#include "generated/protocol.h"
#include "request_layer.h"
#include "status_session.h"
#include "status_view.h"

static Window *s_main_window;
static AppTimer *s_timer;
static TbRequestLayer s_requests;
static TbStatusSession s_session;
static uint32_t s_inbox_size;

static void cancel_timer(void *context) {
  (void)context;
  if (s_timer) { app_timer_cancel(s_timer); s_timer = NULL; }
}
static void on_timeout(void *context) {
  (void)context;
  s_timer = NULL;
  tb_requests_tick(&s_requests);
}
static bool schedule(void *context, uint32_t milliseconds) {
  (void)context;
  s_timer = app_timer_register(milliseconds, on_timeout, NULL);
  return s_timer != NULL;
}
static uint32_t now_ms(void *context) {
  (void)context;
  time_t seconds = 0;
  uint16_t milliseconds = 0;
  time_ms(&seconds, &milliseconds);
  return (uint32_t)seconds * 1000u + milliseconds;
}
static void render(void *context, TbStatus status) { (void)context; tb_status_view_set(status); }
static TbSendResult send_request(void *context, uint8_t kind, uint32_t sequence) {
  (void)context;
  if (!connection_service_peek_pebble_app_connection()) { return TB_SEND_UNREACHABLE; }
  DictionaryIterator *out = NULL;
  const AppMessageResult begun = app_message_outbox_begin(&out);
  if (begun == APP_MSG_NOT_CONNECTED) { return TB_SEND_UNREACHABLE; }
  if (begun != APP_MSG_OK || !out) { return TB_SEND_FAILED; }
  if (dict_write_uint8(out, MESSAGE_KEY_REQUEST_KIND, kind) != DICT_OK ||
      dict_write_int(out, MESSAGE_KEY_REQUEST_SEQ, &sequence, sizeof(sequence), false) != DICT_OK) { return TB_SEND_FAILED; }
  if (kind == TB_REQUEST_HELLO && dict_write_int(out, MESSAGE_KEY_INBOX_SIZE, &s_inbox_size, sizeof(s_inbox_size), false) != DICT_OK) { return TB_SEND_FAILED; }
  const AppMessageResult sent = app_message_outbox_send();
  return sent == APP_MSG_OK ? TB_SEND_OK : sent == APP_MSG_NOT_CONNECTED ? TB_SEND_UNREACHABLE : TB_SEND_FAILED;
}
static bool int32_tuple(const Tuple *tuple) { return tuple && tuple->type == TUPLE_INT && tuple->length == sizeof(int32_t); }
static void inbox_received(DictionaryIterator *iter, void *context) {
  (void)context;
  Tuple *kind = dict_find(iter, MESSAGE_KEY_RESPONSE_KIND);
  Tuple *sequence = dict_find(iter, MESSAGE_KEY_REQUEST_SEQ);
  Tuple *result = dict_find(iter, MESSAGE_KEY_RESULT_CODE);
  const bool valid = int32_tuple(kind) && int32_tuple(sequence) && int32_tuple(result);
  tb_status_session_receive(&s_session, valid, valid ? kind->value->int32 : 0, valid ? sequence->value->int32 : 0, valid ? result->value->int32 : 0);
}
static void outbox_failed(DictionaryIterator *iter, AppMessageResult reason, void *context) {
  (void)context;
  Tuple *sequence = dict_find(iter, MESSAGE_KEY_REQUEST_SEQ);
  if (!sequence || sequence->type != TUPLE_UINT || sequence->length != sizeof(uint32_t)) { return; }
  tb_requests_send_failed(&s_requests, sequence->value->uint32, reason == APP_MSG_NOT_CONNECTED);
}
static void select_clicked(ClickRecognizerRef recognizer, void *context) {
  (void)recognizer; (void)context;
  tb_status_session_retry(&s_session);
}
static void click_config(void *context) {
  (void)context;
  window_single_click_subscribe(BUTTON_ID_SELECT, select_clicked);
}
static void init(void) {
  const TbRequestPorts request_ports = {send_request, schedule, cancel_timer, now_ms, NULL};
  tb_requests_init(&s_requests, request_ports);
  const TbStatusPorts ports = {render, NULL};
  tb_status_session_init(&s_session, &s_requests, ports);
  s_main_window = tb_status_view_create();
  window_set_click_config_provider(s_main_window, click_config);
  window_stack_push(s_main_window, true);
  app_message_register_inbox_received(inbox_received);
  app_message_register_outbox_failed(outbox_failed);
  s_inbox_size = app_message_inbox_size_maximum();
  tb_status_session_start(&s_session, app_message_open(s_inbox_size, app_message_outbox_size_maximum()) == APP_MSG_OK);
}
static void deinit(void) {
  tb_status_session_stop(&s_session);
  tb_requests_cancel_all(&s_requests);
  app_message_deregister_callbacks();
  tb_status_view_destroy(s_main_window);
}
int main(void) { init(); app_event_loop(); deinit(); }
