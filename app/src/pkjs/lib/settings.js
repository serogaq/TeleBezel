...
function validate(raw) {
  ...
  if (!value.address || !value.token) { return {ok: false, missing: true}; }
...
function fromClay(values) { return normalize({address: clayValue(values, 'CONFIG_ADDRESS'), ssl: clayValue(values, 'CONFIG_SSL'), token: clayValue(values, 'CONFIG_TOKEN')}); }
function toClay(value) { value = normalize(value); return {CONFIG_ADDRESS: value.address, CONFIG_SSL: value.ssl, CONFIG_TOKEN: value.token}; }
function baseUrl(value) { var valid = validate(value); return valid.ok ? (valid.settings.ssl ? 'https://' : 'http://') + valid.settings.address : null; }
module.exports = {STORAGE_KEY: STORAGE_KEY, normalize: normalize, validate: validate, load: load, save: save, fromClay: fromClay, toClay: toClay, baseUrl: baseUrl};
