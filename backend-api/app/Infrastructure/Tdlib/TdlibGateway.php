...
namespace App\Infrastructure\Tdlib;
...
final class TdlibGateway implements TdlibGatewayContract
{
    ...
    {
        return $this->request('GET', '/internal/v1/accounts', [
    ...
    {
        return $this->request('GET', "/internal/v1/accounts/{$accountId}", [], $requestId);
    }
    ...
    {
        return $this->request('POST', "/internal/v1/accounts/{$accountId}/proxy/ping", [
            'proxy' => $proxy,
        ], $requestId);
    }
    ...
    {
        return $this->request('GET', "/internal/v1/accounts/{$accountId}/chats", $query, $requestId);
    }
    ...
    {
        return $this->request('GET', "/internal/v1/accounts/{$accountId}/chats/{$chatId}", $query, $requestId);
    }
    ...
    {
        return $this->request('GET', "/internal/v1/accounts/{$accountId}/chats/{$chatId}/messages", $query, $requestId);
    }
    ...
    {
        return $this->request('GET', "/internal/v1/accounts/{$accountId}/updates", $query, $requestId);
    }
    ...
    {
        $this->request('DELETE', '/internal/v1/interests/principal', [
            'principal_type' => $type,
            'principal_id' => $id,
        ], $requestId);
    }
    ...
    private function client(string $requestId): PendingRequest
    {
        ...
        ])->acceptJson()->asJson()->connectTimeout(1)->timeout(10);
    }

    /** @param array<string, mixed> $payload
     * @return array<string, mixed>
     */
    private function request(string $method, string $path, array $payload, string $requestId): array
    {
        try {
            $response = $this->client($requestId)->send($method, $path, $method === 'GET' ? [
                'query' => $payload,
            ] : [
                'json' => $payload,
            ]);
