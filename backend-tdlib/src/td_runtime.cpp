...
struct TdRuntime::Impl {
  ...
  Impl(const Config &config, AccountRegistry &registry, std::unique_ptr<TdTransport> transport)
      : transport(transport ? std::move(transport) : std::make_unique<NativeTdTransport>()), broker(*this->transport),
        cursors(config.internal_token), proxy(context, config, broker),
        lifecycle(context, config, registry, *this->transport, broker, proxy),
        authorization(context, broker, lifecycle), reads(context, config, broker, cursors),
        interests(context, *this->transport, broker), engine(context, registry, *this->transport, broker) {}
};
...
nlohmann::json TdRuntime::logout(const std::string &uuid, const nlohmann::json &command) {
  return impl_->lifecycle.logout(uuid, runtime::AccountCommand::parse(uuid, command));
}
...
nlohmann::json TdRuntime::remove(const std::string &uuid, const nlohmann::json &command) {
  return impl_->lifecycle.remove(uuid, runtime::AccountCommand::parse(uuid, command));
}
...
nlohmann::json TdRuntime::set_interest(const std::string &uuid, std::int64_t chat_id, const std::string &lease_key,
                                       bool active) {
  return impl_->interests.set_interest(uuid, chat_id, lease_key, active);
}
